...
namespace CoreSpatial.ShpOper
{
    ...
    internal class ShpReader
    {
        public ShpReader(string shpFilePath, List<ShxRecord> shxRecords)
        {
            ...
            readStream = new FileStream(shpFilePath, FileMode.Open, FileAccess.Read, FileShare.Read);
            this.ShpHeader = ShpUtility.GetHeader(readStream);
        }
        ...
        public byte[] GetNextRecord()
        {
            ...
            if (this._shxRecords != null && this._shxRecords.Count > 0)
            {
                ...
                return GetNextRecord(_shxRecords[_currentRecord - 1].Offset * 2 + 8, _shxRecords[_currentRecord - 1].Length * 2);
            }
            else
            {
